...
This document describes the end-to-end (e2e) test strategy for QuickView using
...
|---|-----------|----------------|
| S1 | Homepage loads | `<title>` contains "QuickView" |
...
| # | Test Case | Expected Result |
|---|-----------|----------------|
| FP6 | Run button executes script | Output panel shows "Hello, QuickView!" |
...
- name: Install dependencies
  run: npm ci
  working-directory: quickview-tool
...
- name: Upload Playwright report
  ...
  with:
    ...
    path: quickview-tool/tests/playwright-report/
    retention-days: 30
```
