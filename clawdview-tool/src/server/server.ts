...
interface ClawdViewServerOptions {
  port?: number;
  host?: string;
  watchDir?: string;
  autoOpen?: boolean;
...
export class ClawdViewServer {
  ...
  constructor(options: ClawdViewServerOptions = {}) {
  ...
  private async findAvailablePort(startPort: number, maxAttempts = 20): Promise<number> {
    for (let i = 0; i < maxAttempts; i++) {
      ...
      if (await ClawdViewServer.isPortAvailable(port, this.host)) {
        return port;
      }
      if (i === 0) {
